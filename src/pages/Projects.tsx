...
import { RefreshCw, Filter, Play, Sparkles } from 'lucide-react';
...
export default function Projects() {
  ...
  const filtered = projects.filter((p) =>
    search ? p.name.toLowerCase().includes(search.toLowerCase()) || p.id.toLowerCase().includes(search.toLowerCase()) : true
  );
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              ...
              {filtered.map((project) => (
                <tr key={project.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="text-sm text-slate-800">{project.clientId}</div>
                  </td>
                  <td className="px-4 py-3">
